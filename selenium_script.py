...
def cafe24_register(driver, id, pw, name, phone, mail):
  ...
  if '-' in phone:
    ...
    phone_number = phone.replace('010', '')
    middle_digits = phone_number[:4]
    last_digits = phone_number[4:]
    
    driver.find_element(By.ID, "mobile2").send_keys(middle_digits)
    time.sleep(0.5)
    driver.find_element(By.ID, "mobile3").send_keys(last_digits)
    time.sleep(0.5)
